...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* nao seja um ghost`
				...
				teks = `finalmente a carga do grupo é reduzida 𝟭,bye tchau🥳 @${num.split('@')[0]} Seus serviços serão enterrados profundamente²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
            ...
            break
				...
				if (getLevelingLevel(sender) < ahf ) return reply(ind.reglevelahf(command, pushname, getLevelingLevel, sender, ahf))
					...
					const bisa =['lata','Não podes','Experimentar Repetir']
				...
				if (getLevelingLevel(sender) < ahf ) return reply(ind.reglevelahf(command, pushname, getLevelingLevel, sender, ahf))
					...
					const kapan =['Amanhã','Nunca','Agora mesmo','restam 4 dias','Mais 5 dias','Mais 6 dias','1 semana novamente','2 semanas de novo','3 Outra semana','1 A lua de novo','2 A lua de novo','3 A lua de novo','4 A lua de novo','5 A lua de novo','6 A lua de novo','1 Tahun Lagi','2 Outro ano','Mais 3 anos','Mais 4 anos','Mais 5 anos','Mais 6 anos','1 século novamente','Mais 3 dias']
           ...
           if (getLevelingLevel(sender) < ahf ) return reply(ind.reglevelahf(command, pushname, getLevelingLevel, sender, ahf))
					apakah = body.slice(1)
					const apa =['sim','Não','Pode ser','Tente repetir']
...
queios* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}`
                ...
                if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					var wm = gh.split("|")[1];
					var bg = gh.split("|")[2];
					const pref = `Uso: \n${prefix}citarmaker texto|marca d'água|tema\n\nEx :\n${prefix}citarmaker esta exemplo|bicit|aleatória`
                 ...
                 if (getLevelingLevel(sender) < ahe ) return reply(ind.reglevelahe(command, pushname, getLevelingLevel, sender, ahe))
					...
					break
        /*        case 'mechute':
          *      if (isGroupAdmins) return reply(`Desculpa ${pushname} Eu não consigo sair do admin`)
           *     reply(`Por quê ${pushname} Saída?\nok se isso quiser tchau tchau`)
           *     await client.groupRemove(groupId, sender)
                  break*/
                case 'verdade':
                if (!isRegistered) return reply(ind.noregis())
                if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					const trut =['Já gostou de alguém? quanto tempo? ',' Se possível ou se você quiser, no gc / fora do gc quem você fará amigos? (pode ser do tipo / sma diferente)','Qual é o seu maior medo?', 'Você já gostou de alguém e sentiu que essa pessoa também gostava de você?', 'Qual é o nome da ex-namorada do seu amigo de que você gostava secretamente?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
			...
			if (getLevelingLevel(sender) < ahd ) return reply(ind.reglevelahd(command, pushname, getLevelingLevel, sender, ahd))
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('é o texto que significa bambang😤')
				...
				case 'transmitir': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ AVISO LIGHTBOT 👑✨ ❯\n\n${body.slice(4)}`})
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 AVISO LIGHT BOT 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
				...
				case 'adicionar':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('cade o número corno?')
            ...
            break
				case 'bem-vindo':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ DESATIVO,,NESTE GRUPO')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ ATIVO,,NESTE GRUPO')
					} else {
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('clonado✌️')
